...
local requestsDisabled = true --getgenv and getgenv().DISABLE_BloxX_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local BloxXFolder = "BloxX"
local ConfigurationFolder = BloxXFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in just make getSetting(name)
		BloxXOpen = {Type = 'bind', Value = 'K', Name = 'BloxX Keybind'},
		-- buildwarnings
		-- BloxXprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.BloxXOpen"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(BloxXFolder) then
				if isfile and isfile(BloxXFolder..'/settings'..ConfigurationExtension) then
					file = readfile(BloxXFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"BloxXOpen":{"Value":"K","Type":"bind","Name":"BloxX Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"BloxX Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}]
			end
	...
	if not success then 
		if writefile then
			warn('BloxX had an issue accessing configuration saving capability.')
		end
	end
end
...
local BloxXLibrary = {
	...
	Theme = {
		...
		ModernLight = {
			...
			TabStroke = Color3.fromRGB(210, 210, 210),
			...
			ToggleDisabled = Color3.fromRGB(180, 180, 180),
			ToggleEnabledStroke = Color3.fromRGB(0, 122, 255),
			ToggleDisabledStroke = Color3.fromRGB(160, 160, 160),
			ToggleEnabledOuterStroke = Color3.fromRGB(200, 200, 200),
			ToggleDisabledOuterStroke = Color3.fromRGB(190, 190, 190),

			DropdownSelected = Color3.fromRGB(235, 235, 235),
			DropdownUnselected = Color3.fromRGB(245, 245, 245),
...
local BloxX = useStudio and script.Parent:FindFirstChild('BloxX') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local BloxXDestroyed = false -- True when BloxXLibrary:Destroy() is called

repeat
	if BloxX:FindFirstChild('Build') and BloxX.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('BloxX | Build Mismatch')
		print('BloxX may encounter issues as you are running an incompatible interface version ('.. ((BloxX:FindFirstChild('Build') and BloxX.Build.Value) or 'No Build') ..').\n\nThis version of BloxX is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, BloxX = BloxX, useStudio and script.Parent:FindFirstChild('BloxX') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(BloxX)
	BloxX.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	BloxX.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == BloxX.Name and Interface ~= BloxX then
			Interface.Enabled = false
			Interface.Name = "BloxX-Old"
		end
	end
elseif not useStudio then
...
if BloxX.AbsoluteSize.X < minSize.X and BloxX.AbsoluteSize.Y < minSize.Y then
...
local Main = BloxX.Main
local MPrompt = BloxX:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = BloxX:FindFirstChild('Drag')
...
local Notifications = BloxX.Notifications

local SelectedTheme = BloxXLibrary.Theme.ModernDark  -- Changed default to ModernDark

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = BloxXLibrary.Theme[Theme]
	...
	Main.Search.Shadow.Image.ImageColor3 = SelectedTheme.TextColor
	Main.Search.Search.Image.ImageColor3 = SelectedTheme.TextColor
	...
	for _, text in ipairs(BloxX:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("BloxX | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("BloxX | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('BloxX had an issue decoding the configuration file, please try delete the file and reopen BloxX.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(BloxXLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("BloxX | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--BloxXLibrary:Notify({Title = "BloxX Flags", Content = "BloxX was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(BloxXLibrary.Flags) do
...
function BloxXLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('BloxX | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			BloxXLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			BloxXLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "BloxXOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(BloxXFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('BloxX Settings', 0, true)

	if TabList['BloxX Settings'] then
		TabList['BloxX Settings'].LayoutOrder = 1000
	end

	if Elements['BloxX Settings'] then
		Elements['BloxX Settings'].LayoutOrder = 1000
	end
...
function BloxXLibrary:CreateWindow(Settings)
	print('creating window')
	if BloxX:FindFirstChild('Loading') then
		if getgenv and not getgenv().BloxXCached then
			BloxX.Enabled = true
			BloxX.Loading.Visible = true

			task.wait(1.4)
			BloxX.Loading.Visible = false
		end
	end

	if getgenv then getgenv().BloxXCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				BloxXLibrary:Notify({Title = 'Build Mismatch', Content = 'BloxX may encounter issues as you are running an incompatible interface version ('.. ((BloxX:FindFirstChild('Build') and BloxX.Build.Value) or 'No Build') ..').\n\nThis version of BloxX is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if Settings.ShowText then
		MPrompt.Title.Text = 'UI '..Settings.ShowText
	end

	LoadingFrame.Version.TextTransparency = 1
	LoadingFrame.Title.Text = Settings.LoadingTitle or "BloxX"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "BloxX Interface Suite" then
		LoadingFrame.Version.Text = "BloxX UI"
	end
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(BloxXFolder.."/Discord Invites") then
			makefolder(BloxXFolder.."/Discord Invites")
		end

		if isfile and not isfile(BloxXFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(BloxXFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"BloxX RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(BloxXFolder.."/Key System") then
			makefolder(BloxXFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("BloxX | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BloxX specific development.')
				end
			end
		end
		...
		if isfile and isfile(BloxXFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(BloxXFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(BloxXFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						BloxXLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			BloxXLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			BloxXLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('BloxX had an issue creating settings.') end
...
function BloxXLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function BloxXLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function BloxXLibrary:Destroy()
	BloxXDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['BloxX Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "BloxXOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function BloxXLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				BloxXLibrary:Notify({Title = "BloxX Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			BloxXLibrary:Notify({Title = "BloxX Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('BloxX Configurations Error | '..tostring(result))
			BloxXLibrary:Notify({Title = "BloxX Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
task.delay(4, function()
	BloxXLibrary.LoadConfiguration()
